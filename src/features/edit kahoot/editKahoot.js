...
const EditKahut = ({ match }) => {
  ...
  useEffect(() => {
    if (isSuccess) {
      console.log(curRoom);
      return setListQuestion([...curRoom.questions]);
    }
  }, [isSuccess]);

  function handleChangeImageUpload(event) {
            ...
            >
              <Typography noWrap sx={{ px: 1 }}>
                {listQuestion[curQuestion].time / 60 >= 2 &&
                  Math.floor(listQuestion[curQuestion].time / 60) + " minutes "}
                {listQuestion[curQuestion].time / 60 === 1 &&
                  Math.floor(listQuestion[curQuestion].time / 60) + " minute "}
                {listQuestion[curQuestion].time % 60 >= 1 &&
                  (listQuestion[curQuestion].time % 60) + " seconds"}
              </Typography>
              <Box
                sx={{
                  flexGrow: 1,
